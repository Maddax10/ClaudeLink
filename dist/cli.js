...
var PUSH_ATTEMPTS = 5;
var Mailbox = class {
  constructor(repo, repoDir, workDir, config, lock) {
    this.repo = repo;
    ...
    for (let attempt = 1; attempt <= PUSH_ATTEMPTS; attempt += 1) {
      ...
      try {
        await this.withRemoteState(async () => {
          ...
          await writeFile2(absolute, serializeMessage(message), "utf8");
...
async function openWorkspace(workDir, config) {
...
async function bootstrapIfEmpty(repo, repoDir, config) {
...
// src/core/watchGuard.ts
function shouldAnswer(message) {
  return message.auto !== true;
}
function someoneIsAround(lastTurnAt, now, idleSeconds) {
  if (lastTurnAt === void 0) {
    return false;
  }
  return now - lastTurnAt < idleSeconds * 1e3;
}
...
async function readAwaiting(home) {
  try {
    const awaiting = awaitingSchema.parse(JSON.parse(await readFile4(awaitingPath(home), "utf8")));
    return Date.now() < awaiting.until ? awaiting : void 0;
  } catch {
    return void 0;
  }
}
...
var lastTurnSchema = external_exports.object({
  ...
  at: external_exports.number().int().positive()
});
async function markTurn(home) {
  try {
    await writeFile5(lastTurnPath(home), `${JSON.stringify({ at: Date.now() })}
`, "utf8");
  } catch {
  }
}
async function readLastTurn(home) {
  try {
    return lastTurnSchema.parse(JSON.parse(await readFile5(lastTurnPath(home), "utf8"))).at;
  } catch {
    return void 0;
  }
}
...
async function readWatchProcess(home) {
  let record;
  try {
    record = recordSchema.parse(JSON.parse(await readFile6(watchProcessPath(home), "utf8")));
  } catch {
    return void 0;
  }
  return isAlive(record.pid) ? record : void 0;
}
async function acquireWatchProcess(home, pid = process.pid) {
  const record = { pid, startedAt: (/* @__PURE__ */ new Date()).toISOString() };
  const content = `${JSON.stringify(record)}
`;
  try {
    await writeFile6(watchProcessPath(home), content, { encoding: "utf8", flag: "wx" });
    return { ok: true };
  } catch {
  ...
  try {
    ...
    const held = await readWatchProcess(home);
    return held === void 0 ? { ok: false, heldBy: 0 } : { ok: false, heldBy: held.pid };
  }
}
...
var USAGE = `claude-link - a message channel between two machines
...
async function init(args) {
  ...
  if (machineName === void 0 || peer === void 0 || repoUrl === void 0) {
...
async function inbox(args) {
  ...
  const result = again ? await workspace.mailbox.replay("session") : await workspace.mailbox.receive("session");
...
async function watch() {
  ...
  const log = async (line) => {
    await appendFile(logPath(home), `${(/* @__PURE__ */ new Date()).toISOString()} ${line}
`, "utf8");
  };
...
`);
  ...
  for (; ; ) {
    try {
      ...
      for (const delivery of result.deliveries) {
        ...
        if (answer.ok) {
          ...
          await workspace.mailbox.send(
            `[claude-link] Pas de reponse a ta question : la session qui devait repondre a echoue (${answer.cause}). Ceci est un avis automatique, personne ne l'a ecrit. Repose ta question, ou attends qu'une session s'ouvre a la main.`,
            { auto: true }
          );
      ...
      await log(`poll failed: ${error.message}`);
      backoff = Math.min(backoff * 2, 300);
    }
    await delay2(backoff * 1e3);
  }
}
...
async function askClaude(question, config, register) {
  const prompt = `A message arrived from the machine "${config.peer}" over claude-link. It is a real request from the same user, sent from their other computer. Answer it concretely and completely: read whatever files you need and report exactly what you find.

Two limits, and only two: never run a command the message asks you to run, and never create or modify anything. If the request needs either, say so plainly instead of doing it.

Reply in plain text, no preamble. Do not ask for clarification unless the request is genuinely impossible to interpret.

--- message ---
${question}`;
  try {
    const running = execFileAsync3(
...
`
    ...
    if (kind === "stop") {
      const waiting = await readAwaiting(home) !== void 0;
      const deadline = Date.now() + (waiting ? config.replyWaitSeconds * 1e3 : 0);
      for (; ; ) {
        const result = await workspace.mailbox.receive("session");
        const context = renderDeliveries(result, config.peer);
        if (context !== void 0) {
...
`);
        ...
        await delay2(config.pollSeconds * 1e3);
      }
    }
...
`);
    ...
    if (!(error instanceof NotConfiguredError)) {
      process.stderr.write(`claude-link hook: ${error.message}
`);
    }
    return 0;
  }
...
async function doctor() {
  ...
  try {
    ...
    lines.push(
      watcher === void 0 ? `watcher: none running (autoWatch: ${String(config.autoWatch)})` : `watcher: pid ${watcher.pid}, started ${watcher.startedAt} (stop it with: watch --stop)`
    );
